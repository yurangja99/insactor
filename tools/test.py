...
def main():
    ...
    if phys_mode == 'normal':
        ...
        for i in tqdm(range(0, len(outputs), num_env)):
            ...
            #######################################################
            # EDIT ZONE
            np.save(f'{args.work_dir}/{phys_setting}{"_perturb" if args.perturb == "true" else ""}/planned_traj.npy', pred_motion.transpose(1,0,2))
            #######################################################
            ...
            #######################################################
            # EDIT ZONE
            np.save(f'{args.work_dir}/{phys_setting}{"_perturb" if args.perturb == "true" else ""}/simulated_traj.npy', pred_motion_phys.transpose(1,0,2))
            #######################################################
